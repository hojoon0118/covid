...
df.covid <- read.csv(file = 'data1/transformed_data.csv', fileEncoding="UTF-8-BOM")
```
...
c1 has Pearson's correlation test revealed that GDPCAP and HDI has (r = `r c1$estimate`, p = `r c1$p.value`)
For c2 has Pearson's correlation test revealed that TC and TD has (r = `r c2$estimate`, p = `r c2$p.value`)
As a last result, c3 has Pearson's correlation test revealed that GDPCAP and TCPOP has (r = `r c3$estimate`, p = `r c3$p.value`)
By performing a linear regression, HDI and GDPCAP showed significant positive main effect (F(1,159) = `HGsum[[1]][1,4]`, p = `HGsum[[1]][1,5]`).
Similarly, TC and TD showed significant significant main effect (F(1,159) = `CDsum[[1]][1,4]`, p = `CDsum[[1]][1,5]`).
Lastly, TCPOP and GDPCAP showed significant main effect. (F(1,159) = `CGsum[[1]][1,4]`, p = `CGsum[[1]][1,5]`).
